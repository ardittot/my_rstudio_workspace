...
df_result <-   runNLOptR(file_historical, file_budget_season, file_to_pred, file_DR_dataset, Total_budget, Season, CA_flight_target, CA_hotel_target, forceMaxBudget=TRUE, num_iter=4000)
...
if ((CA_flight_prev < CA_flight_target) | (CA_hotel_prev < CA_hotel_target)) {
  df_result <- runNLOptR(file_historical, file_budget_season, file_to_pred, file_DR_dataset, Total_budget, Season, CA_flight_target, CA_hotel_target, forceMaxBudget=TRUE, num_iter=1000, init_budget=df_result$budget_channel)
  write.csv(df_result,file_out, row.names = FALSE)
}
